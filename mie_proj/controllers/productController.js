...
exports.getAllProductBrandCategory = function(req, res) {
  
	Product.collection.distinct('product_category',{ product_brand: req.query.brand_name,
			  product_type: req.query.product_type},function(err, products) {
    if (err)
    	res.json({status:'901',message:'Expetion occurred while fetching product category' 
    	  	  ,data:{},error:err});
    else
...
exports.getAllBrandModel = function(req, res) {
	...
	Product.find({ product_brand: req.query.brand_name,
			  product_type: req.query.product_type,product_category:req.query.product_category}).select({product_model :1 ,_id :1}).exec(function(err, products) {
    if (err)
    	res.json({status:'901',message:'Expetion occurred while fetching product type' 
    	  	  ,data:{},error:err});
    else
